...
import { testAdminAuth } from '../../api/adminApiDefinitions'
...
export default function AdminApiDefinitionsPage() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      setDefs(await listAdminApiDefinitions())
    } catch (e: unknown) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId) {
        ...
        const created = await createAdminApiDefinition({
  ...
  async function handleToggleActive(def: ApiDefinitionRecord) {
    ...
    try {
      const updated = def.isActive
        ? await deactivateAdminApiDefinition(def.id)
        : await activateAdminApiDefinition(def.id)
  ...
  async function handleDelete(id: string) {
    ...
    setDeletingId(id)
    try {
      await deleteAdminApiDefinition(id)
    ...
    <AdminShell>
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-white text-xl font-semibold">API Definitions</h1>
            <p className="text-gray-400 text-sm mt-0.5">
              Tenant-level API overrides. These take precedence over platform defaults for each API type.
            </p>
          </div>
          <button
            onClick={openCreate}
        ...
        {!loading && !error && defs.length === 0 && (
          <p className="text-gray-500 text-sm">
            No tenant API definitions. Platform defaults will be used for all API types.
          </p>
        )}
                    ...
                    <tr key={d.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-3">
                          ...
                          <button onClick={() => navigate(`/admin/api-definitions/${d.id}`)} className="text-indigo-400 hover:text-indigo-300 text-xs font-medium transition-colors">Edit</button>
                          <button
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
              ...
              <div>
                ...
                <AuthConfigForm
                  ...
                  onTestAuth={() => testAdminAuth(serializeAuthConfig(form.auth))}
                />
              </div>
            </div>
