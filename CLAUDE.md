...
- **Secrets** — Azure Key Vault (prod), .NET User Secrets (local). Never in config files or source. Setting `KeyVault:VaultUri` makes the Api and Worker read every app secret from Key Vault automatically via the Azure Key Vault configuration provider (`AddAzureKeyVault` in each `Program.cs`) — no code changes needed at any call site. Auth to Key Vault itself goes through `AzureCredentialFactory` (`ContactConnection.Infrastructure/Extensions/`): if `EntraId:ClientSecret` is configured (local dev, via User Secrets) it uses `ClientSecretCredential`; otherwise `DefaultAzureCredential` (Managed Identity — production should NOT configure `EntraId:ClientSecret`, so it always resolves there). Local dev with `KeyVault:VaultUri` unset (the default) is completely unaffected and keeps using User Secrets.
